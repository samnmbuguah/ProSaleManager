...
const PosPage: React.FC = () => {
  ...
  const { cart, addToCart, removeFromCart, updateQuantity, updateUnitPrice, updateUnitType, clearCart, addDeliveryService } = useCart();
  ...
  const handleAddDelivery = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const hasDelivery = cart.items.some(item =>
        item.product && item.product.product_code === 'SRV001'
      );
  ...
  const handleAddProductToCart = (product: Product) => {
    ...
    if (product.product_code === 'SRV001') {
      const hasDelivery = cart.items.some(item => item.product.product_code === 'SRV001');
      if (hasDelivery) {
        toast({
          title: 'Error',
          description: 'Delivery charge already added',
                    ...
                    >
                      <CardContent className="p-4">
                        ...
                        <p className="text-sm font-medium">
                          KSh {calculateUnitPrice(product).toFixed(2)} per piece
                        </p>
                      </CardContent>
                    </Card>
                  ))
            ...
            <CardContent className="h-full flex flex-col p-4">
              ...
              <div className="flex-1 overflow-y-auto">
                <div className="space-y-4">
                  {cart.items.map((item) => (
                    <div key={item.id} className="flex items-start gap-2 pb-4 border-b">
                      <div className="flex-1">
                        ...
                        <div className="mt-2">
                          <Select
                            ...
                            onValueChange={(value) => updateUnitType(item.id, value)}
                            disabled={item.product.product_code === 'SRV001'}
                          >
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setSelectedCustomer(value !== 'walk_in' ? parseInt(value) : null)}
              >
                <SelectTrigger>
