...
export function PurchaseOrders() {
  ...
  const handleStatusChange = async (orderId: number, newStatus: PurchaseOrder['status']) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/purchase-orders/${orderId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
          ...
          {selectedOrder && (
            <div className="space-y-4">
              <div>
                ...
                <p>Order Date: {format(new Date(selectedOrder.order_date), 'MMM d, yyyy')}</p>
                <p>
                  Expected Delivery:{' '}
                ...
                <p>Total Amount: KSh {selectedOrder.total_amount.toLocaleString('en-KE')}</p>
              </div>
              {selectedOrder.notes && (
                <div>
