...
describe("POS Page Tests", () => {
  ...
  test("should complete checkout process", async () => {
    ...
    await driver
      .findElement(By.css('[data-testid="payment-method-cash"]'))
      .click();
    // Complete checkout
    await driver
      .findElement(By.css('[data-testid="complete-checkout"]'))
      .click();
    // Verify success
  ...
  test("should add delivery service", async () => {
    // Add a product first
    const productCard = await waitForElement(
      driver,
      '[data-testid="product-card"]',
    );
    await productCard.click();
    // Click add delivery button
    const deliveryButton = await driver.findElement(
      By.xpath('//button[contains(text(), "Add Delivery")]'),
    );
    await deliveryButton.click();
    // Check if delivery service appears in cart
    const deliveryItem = await driver.wait(
      until.elementLocated(
        By.xpath('//div[contains(text(), "Delivery Service")]'),
      ),
      5000,
    );
