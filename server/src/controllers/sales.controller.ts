...
export const createSale = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const { items, total, payment_method, status, payment_status, amount_paid, change_amount, customer_id } = req.body;
...
export const getSaleItems = async (req: Request, res: Response) => {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'Failed to fetch sale items';
    res.status(500).json({ message: errorMessage });
  }
};
