...
async function main() {
    ...
    // --- Users with environment variables ---
    const users = [
        // Eltee Store
        {
            email: process.env.ELTEE_ADMIN_EMAIL || 'eltee.admin@prosale.com',
            password: process.env.ELTEE_ADMIN_PASSWORD || 'changeme2025',
            name: 'Eltee Admin',
            role: 'admin' as const,
            store_id: elteeStore.id
        },
        {
            email: process.env.ELTEE_MANAGER_EMAIL || 'eltee.manager@prosale.com',
            password: process.env.ELTEE_MANAGER_PASSWORD || 'changeme2025',
            name: 'Eltee Manager',
            role: 'manager' as const,
            store_id: elteeStore.id
        },
        {
            email: process.env.ELTEE_CASHIER_EMAIL || 'eltee.cashier@prosale.com',
            password: process.env.ELTEE_CASHIER_PASSWORD || 'changeme2025',
            name: 'Eltee Cashier',
            role: 'sales' as const,
            store_id: elteeStore.id
        },
        // Eltee CBD Store
        {
            email: process.env.ELTEE_CBD_ADMIN_EMAIL || 'eltee.cbd.admin@prosale.com',
            password: process.env.ELTEE_CBD_ADMIN_PASSWORD || 'changeme2025',
            name: 'Eltee CBD Admin',
            role: 'admin' as const,
            store_id: elteeCbdStore.id
        },
        {
            email: process.env.ELTEE_CBD_MANAGER_EMAIL || 'eltee.cbd.manager@prosale.com',
            password: process.env.ELTEE_CBD_MANAGER_PASSWORD || 'changeme2025',
            name: 'Eltee CBD Manager',
            role: 'manager' as const,
            store_id: elteeCbdStore.id
        },
        {
            email: process.env.ELTEE_CBD_CASHIER_EMAIL || 'eltee.cbd.cashier@prosale.com',
            password: process.env.ELTEE_CBD_CASHIER_PASSWORD || 'changeme2025',
            name: 'Eltee CBD Cashier',
            role: 'sales' as const,
            store_id: elteeCbdStore.id
        },
    ];
    for (const u of users) {
        // Pass plain text password; User model hooks will hash it once
        await User.create({
            email: u.email,
            password: u.password,
            name: u.name,
    ...
    // --- CSV product import ---
    // In production, the CSV lives at the project root (outside dist),
