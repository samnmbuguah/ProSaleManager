...
export const seedProducts = async (): Promise<void> => {
  try {
    ...
    for (const store of stores) {
      ...
      console.log(`\nCreated products for store ${store.name}:`, JSON.stringify(productsToCreate, null, 2));
      ...
      console.log(`\ud83c\udf89 Advanced product seeder completed successfully for store ${store.name}!`);
      ...
      if (finalCount !== productsToCreate.length) {
        console.warn(`\u26a0\ufe0f  Warning: Expected ${productsToCreate.length} products but found ${finalCount} in database for store ${store.name}`);
      }
    }
  } catch (error) {
